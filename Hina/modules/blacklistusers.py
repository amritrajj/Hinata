...
BLACKLISTWHITELIST = [OWNER_ID] + list(DEV_USERS) + list(DRAGONS) + list(WOLVES) + list(DEMONS)
...
def __user_info__(user_id: int) -> str:
    if user_id in {777000, 1087968824} or user_id == app.bot.id:
        return ""
    if user_id in DRAGONS + TIGERS + WOLVES:
        return ""
    
    is_blacklisted = sql.is_user_blacklisted(user_id)
    text = "Blacklisted: <b>Yes</b>" if is_blacklisted else "Blacklisted: <b>No</b>"
...
app.add_handler(BL_HANDLER)
app.add_handler(UNBL_HANDLER)
app.add_handler(BLUSERS_HANDLER)
