...
from telegram.ext import CommandHandler, MessageHandler, filters, ContextTypes, BaseHandler
from telegram.helpers import mention_html
from contextlib import asynccontextmanager

import Hina.modules.sql.blacklist_sql as sql
...
async def blacklist(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if conn:
        chat_id = conn
        chat_name = (await context.bot.getChat(conn)).title
    else:
...
async def blacklist_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args:
        if args[0].lower() in ["off", "nothing", "no"]:
            ...
            if len(args) == 1:
                teks = """It looks like you tried to set time value for blacklist but you didn't specify time; Try, `/blacklistmode tban <timevalue>`.
...
Example of time value: 4m = 4 minutes, 3h = 3 hours, 6d = 6 days, 5w = 5 weeks."""
            ...
            if len(args) == 1:
                teks = """It looks like you tried to set time value for blacklist but you didn't specify time; try, `/blacklistmode tmute <timevalue>`.
...
BLACKLIST_DEL_HANDLER = MessageHandler(
    (filters.TEXT | filters.COMMAND | filters.Sticker.ALL | filters.PHOTO | filters.Caption) &
    filters.ChatType.GROUPS,
    del_blacklist
)

app.add_handler(BLACKLIST_HANDLER)
app.add_handler(ADD_BLACKLIST_HANDLER)
app.add_handler(UNBLACKLIST_HANDLER)
app.add_handler(BLACKLISTMODE_HANDLER)
app.add_handler(BLACKLIST_DEL_HANDLER, group=BLACKLIST_GROUP)
