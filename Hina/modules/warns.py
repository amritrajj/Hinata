...
from Hina.modules.helper_funcs.string_handling import split_quotes
from Hina.modules.log_channel import loggable
from Hina.modules.sql import warns_sql as sql
...
def warn(
    ...
    if num_warns >= limit:
        ...
        if soft_warn:  # punch
            chat.ban_member(user.id, until_date=60)
            reply = (
...
async def reset_warns(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    if user_id:
        ...
        warned = await chat.get_member(user_id)
        return (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#RESETWARNS\n"
            f"<b>Admin:</b> {mention_html(user.id, user.first_name)}\n"
            f"<b>User:</b> {mention_html(warned.user.id, warned.user.first_name)}"
        )
    else:
...
async def add_warn_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for handler in context.application.handlers.get(WARN_HANDLER_GROUP, []):
        if handler.filters == (keyword, chat.id):
            context.application.remove_handler(handler, WARN_HANDLER_GROUP)
...
async def list_warn_filters(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for keyword in all_handlers:
        ...
        if len(entry) + len(filter_list) > 4096:
...
WARN_HANDLER = CommandHandler(["warn", "dwarn"], warn_user, filters=filters.ChatType.GROUPS)
...
LIST_WARN_HANDLER = DisableAbleCommandHandler(
    ["warnlist", "warnfilters"], list_warn_filters, filters=filters.ChatType.GROUPS, admin_ok=True,
)
...
app.add_handler(RESET_WARN_HANDLER)
app.add_handler(MYWARNS_HANDLER)
app.add_handler(ADD_WARN_HANDLER)
app.add_handler(RM_WARN_HANDLER)
app.add_handler(LIST_WARN_HANDLER)
app.add_handler(WARN_LIMIT_HANDLER)
app.add_handler(WARN_STRENGTH_HANDLER)
app.add_handler(WARN_FILTER_HANDLER, WARN_HANDLER_GROUP)
