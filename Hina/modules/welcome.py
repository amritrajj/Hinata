...
from .spamwatch import (
    spamwatch_client,
    get_spamwatch_log_setting,
    is_spamwatch_enabled
)

import Hina.modules.sql.welcome_sql as sql
...
from Hina.modules.helper_funcs.msg_types import get_welcome_type
...
# do not async
def send(update: Update, message: str, keyboard: InlineKeyboardMarkup, backup_message: str):
    ...
    if cleanserv:
        try:
            app.bot.delete_message(chat.id, update.message.message_id)
        except BadRequest:
            pass
        reply = False
    try:
...
async def new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for new_mem in new_members:
        if new_mem.id == bot.id and not Hina.config.ALLOW_CHATS:
        ...
        data = None
        if spamwatch_client:  # Check if SpamWatch is available
            try:
               sw_ban = spamwatch_client.get_ban(new_mem.id)
               if sw_ban:
                  await update.effective_chat.ban_member(new_mem.id)
                  if get_spamwatch_log_setting(str(update.effective_chat.id)):
                     log_msg = (
                       f"#SPAMWATCH_BAN\n"
                       f"• User: {mention_html(new_mem.id, new_mem.first_name)}\n"
                       f"• ID: <code>{new_mem.id}</code>\n"
                       f"• Reason: <code>{sw_ban.reason}</code>\n"
               ...
               LOGGER.error(f"Error checking SpamWatch ban: {e}")
        ...
        if should_welc:
            ...
            if cleanserv:
                try:
                    await bot.delete_message(chat.id, update.message.message_id)
                except BadRequest:
                    pass
                reply = False
            ...
            # Welcome yourself
                ...
                if cust_welcome:
                    if cust_welcome == sql.DEFAULT_WELCOME:
                        ...
                        ).format(first=escape_markdown_v2(first_name))

                    if new_mem.last_name:
                        fullname = escape_markdown_v2(f"{first_name} {new_mem.last_name}")
                    else:
                        fullname = escape_markdown_v2(first_name)
                    count = await chat.get_member_count()
                    mention = mention_markdown(new_mem.id, escape_markdown_v2(first_name))
                    if new_mem.username:
                        username = "@" + escape_markdown_v2(new_mem.username)
                    else:
                        username = mention
                    ...
                    res = valid_format.format(
                        first=escape_markdown_v2(first_name),
                        last=escape_markdown_v2(new_mem.last_name or first_name),
                        fullname=escape_markdown_v2(fullname),
                        username=username,
                        mention=mention,
                        count=count,
                        chatname=escape_markdown_v2(chat.title),
                        id=new_mem.id,
                    )
...
async def check_not_bot(context: ContextTypes.DEFAULT_TYPE):
    job = context.job
    member = job.data.get("member")
    chat_id = job.data.get("chat_id")
    message_id = job.data.get("message_id")
    bot = context.bot
    
    member_dict = VERIFIED_USER_WAITLIST.pop(member.id, None)
    if not member_dict:
        return
...
async def left_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if spamwatch_client:
                try:
                   sw_ban = spamwatch_client.get_ban(left_mem.id)
                   if sw_ban:
                      return
                except Exception as e:
                   LOGGER.error(f"Error checking SpamWatch ban: {e}")
            ...
            if cust_goodbye:
                ...
                if left_mem.last_name:
                    fullname = escape_markdown_v2(f"{first_name} {left_mem.last_name}")
                else:
                    fullname = escape_markdown_v2(first_name)
                count = await chat.get_member_count()
                mention = mention_markdown(left_mem.id, first_name)
                if left_mem.username:
                    username = "@" + escape_markdown_v2(left_mem.username)
                else:
                    username = mention
                ...
                res = valid_format.format(
                    first=escape_markdown_v2(first_name),
                    last=escape_markdown_v2(left_mem.last_name or first_name),
                    fullname=escape_markdown_v2(fullname),
                    username=username,
                    mention=mention,
                    count=count,
...
WELC_HELP_TXT = (
    ...
    "buttons.\n"
    f"To create a button linking to your rules, use this: `[Rules](buttonurl://t.me/Narutoxhinatabot?start=group_id)`. "
...
app.add_handler(NEW_MEM_HANDLER)
app.add_handler(LEFT_MEM_HANDLER)
app.add_handler(WELC_PREF_HANDLER)
app.add_handler(GOODBYE_PREF_HANDLER)
app.add_handler(SET_WELCOME)
app.add_handler(SET_GOODBYE)
app.add_handler(RESET_WELCOME)
app.add_handler(RESET_GOODBYE)
app.add_handler(CLEAN_WELCOME)
app.add_handler(WELCOME_HELP)
app.add_handler(WELCOMEMUTE_HANDLER)
app.add_handler(CLEAN_SERVICE_HANDLER)
app.add_handler(BUTTON_VERIFY_HANDLER)
app.add_handler(WELCOME_MUTE_HELP)
