...
from Hina.modules.sql import log_channel_sql as sql
from Hina.modules.helper_funcs.chat_status import user_admin
from Hina.config import EVENT_LOGS, LOGGER, app
    ...
    def log_action(
        ...
        **kwargs,
        ...
        if result and isinstance(result, str):
            ...
            result += f"\n<b>Event Stamp</b>: <code>{
    datetime.utcnow().strftime(datetime_fmt)}</code>"

            if message.chat.type == chat.SUPERGROUP and message.chat.username:
                result += f'\n<b>Link:</b> <a href="https://t.me/{
    chat.username}/{
        message.message_id}">click here</a>'
...
def logging(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if log_channel:
        ...
        message.reply_text(
            ...
            f" {escape_markdown_v2(log_channel_info.title)} (`{log_channel}`)",
...
def __chat_settings__(chat_id, user_id):
    ...
    if log_channel:
        log_channel_info = app.bot.get_chat(log_channel)
        return f"This group has all it's logs sent to: {escape_markdown_v2(log_channel_info.title)} (`{log_channel}`)"
...
app.add_handler(SET_LOG_HANDLER)
app.add_handler(UNSET_LOG_HANDLER)
